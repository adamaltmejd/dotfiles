...
source "$SCRIPT_DIR/bootstrap/lib/detect.sh"
# shellcheck source=bootstrap/lib/link.sh
source "$SCRIPT_DIR/bootstrap/lib/link.sh"
...
usage() {
    cat <<'USAGE'
Usage:
  ./bootstrap.sh --profile <server|local> [--dry-run|--apply] [options]
...
if [[ "$APPLY" -eq 0 ]]; then
    log_info "Dry-run only. Re-run with --apply to execute."
fi
...
if [[ "$DOTFILES_FEAT_CLAUDE" -eq 1 ]]; then
    ...
    link_file "$DOTFILES_DIR/claude/settings.json" "$HOME/.claude/settings.json" "$BACKUP_DIR"
    link_file "$DOTFILES_DIR/claude/CLAUDE.md" "$HOME/.claude/CLAUDE.md" "$BACKUP_DIR"
    link_file "$DOTFILES_DIR/codex/config.toml" "$HOME/.codex/config.toml" "$BACKUP_DIR"
    link_file "$DOTFILES_DIR/codex/rules/default.rules" "$HOME/.codex/rules/default.rules" "$BACKUP_DIR"
    link_file "$DOTFILES_DIR/codex/AGENTS.md" "$HOME/.codex/AGENTS.md" "$BACKUP_DIR"
else
    log_info "Skipping claude/codex dotfiles (use --with-claude to include)."
fi
...
if [[ "$SKIP_PACKAGES" -eq 1 ]]; then
    ...
    install_packages \
        "$DOTFILES_DIR/bootstrap/packages/packages.shared.txt" \
        "$DOTFILES_DIR/bootstrap/packages/packages.$PROFILE.txt" \
        "$PKG_MANAGER" \
        "$SUDO_OK" \
        "$APPLY"
